...
public class FacultyMarkingStatusTable extends DatabaseTable{

	@Override
	public void retrive(String queryCondition) {
		if(queryCondition==null)
		{
		sql="select * from faculty_marking_status";
		...
		{
			sql="select * from faculty_marking_status Where "+queryCondition;
			//sql="select '"+queryAttribute+"' from Detail where "+queryCondition;
			//sql="select * from Detail where Id='admin'";
			try {
				stmpt=con.prepareStatement(sql);
	...
	public void insert(String queryAttributeValues) {
		sql="INSERT INTO faculty_marking_status (ID, Course_ID, Years, Status)"+queryAttributeValues;
	...
	public void delete(String queryCondition) {
		sql="delete from faculty_marking_status Where "+queryCondition;
		try {
			stmpt=con.prepareStatement(sql);
			//stmpt.executeUpdate(sql);
			stmpt.executeUpdate();
	...
	public void update(String queryAttributeValues, String queryCondition) {
		sql="update faculty_marking_status set "+queryAttributeValues+" where "+queryCondition;
		try {
			stmpt=con.prepareStatement(sql);
			//stmpt.executeUpdate(sql);
			stmpt.executeUpdate();
